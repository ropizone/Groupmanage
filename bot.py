...
async def panel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin_or_owner and uid != OWNER_ID:
        return await update.message.reply_text(
            "🚫 Only group admins can use /panel!\n"
            "💡 Members can use /stats instead."
        )
...
async def panel_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "show_my_panel":
        ...
        # Auto-discover: check Telegram API for groups where user is admin/creator
        if not owner_groups:
            for cid, info in data["groups"].items():
                try:
